"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const txwrapper_core_1 = require("@axia-core/txwrapper-core");
const test_helpers_1 = require("../../test-helpers");
const approveAsMulti_1 = require("./approveAsMulti");
describe('multisig::approveAsMulti', () => {
    it('should work', () => {
        const unsigned = (0, approveAsMulti_1.approveAsMulti)(test_helpers_1.TEST_METHOD_ARGS.multisig.approveAsMulti, txwrapper_core_1.TEST_BASE_TX_INFO, txwrapper_core_1.AXIA_25_TEST_OPTIONS);
        (0, txwrapper_core_1.itHasCorrectBaseTxInfo)(unsigned);
        expect(unsigned.method).toBe('');
    });
});
//# sourceMappingURL=approveAsMulti.spec.js.map