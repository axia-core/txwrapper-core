"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const txwrapper_core_1 = require("@axia-core/txwrapper-core");
const test_helpers_1 = require("../../test-helpers");
const setKeys_1 = require("./setKeys");
describe('session::setKeys', () => {
    it('should work', () => {
        const unsigned = (0, setKeys_1.setKeys)(test_helpers_1.TEST_METHOD_ARGS.session.setKeys, txwrapper_core_1.TEST_BASE_TX_INFO, txwrapper_core_1.AXIA_25_TEST_OPTIONS);
        (0, txwrapper_core_1.itHasCorrectBaseTxInfo)(unsigned);
        expect(unsigned.method).toBe('');
    });
});
//# sourceMappingURL=setKeys.spec.js.map